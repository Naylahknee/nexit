    ...
    <div>
      ...
      <section className="relative overflow-hidden rounded-[24px] bg-navy-deep px-6 py-8 text-white sm:px-9 sm:py-10">
        ...
        <div className="relative grid items-end gap-8 lg:grid-cols-[1fr_330px]">
          <div>
            <p className="text-xs font-extrabold uppercase tracking-[.22em] text-gold">{NEXIT_LEXICON.map}</p>
            <h1 className="mt-3 max-w-3xl font-display text-4xl font-bold leading-tight sm:text-5xl">
              {initialRegion ? NEXIT_LEXICON.regionTitle : NEXIT_LEXICON.mapTitle}
            </h1>
            <p className="mt-4 max-w-2xl text-base leading-7 text-[#bdcae0]">
              {initialRegion ? `${activeRegion.name}: ${activeRegion.description}` : 'Compare Nextinations through your budget, work setup, priorities, and Community Fit.'}
            </p>
          </div>

          <div className="rounded-[18px] border border-white/12 bg-white/8 p-5 backdrop-blur">
            <div className="flex items-center justify-between">
              <div><p className="text-xs font-bold text-white/55">{NEXIT_LEXICON.readiness}</p><p className="mt-1 text-3xl font-extrabold text-gold">{profile.readinessScore}%</p></div>
        ...
        <aside className="card-surface flex flex-col p-6 sm:p-7">
          ...
          <div className="mt-auto pt-6">
            {initialRegion ? (
              ...
              <Link href={`/nexitnation/${activeRegion.slug}`} className="gold-button w-full">{NEXIT_LEXICON.regionTitle} <ArrowRight size={16} /></Link>
            )}
          </div>
        </aside>
      </div>
      ...
      <section className="mt-8">
        <div className="flex flex-wrap items-end justify-between gap-4">
          <div><p className="text-xs font-extrabold uppercase tracking-[.18em] text-gold-deep">Nexitnation view</p><h2 className="mt-2 font-display text-3xl font-bold text-navy">Choose a Nextination to compare</h2></div>
        ...
        <div className="mt-5 grid gap-4 md:grid-cols-2 xl:grid-cols-4">
          {REGIONS.map((region) => (
            <Link key={region.slug} href={`/nexitnation/${region.slug}`} className={`group rounded-[18px] border p-5 transition hover:-translate-y-0.5 hover:shadow-lg ${region.slug === activeRegion.slug ? 'border-gold bg-gold-soft/45' : 'border-line bg-white'}`}>
              ...
              <span className="mt-5 inline-flex items-center gap-1 text-xs font-extrabold text-gold-deep">{NEXIT_LEXICON.regionTitle} <ArrowRight size={13} className="transition group-hover:translate-x-0.5" /></span>
            </Link>
          ))}
        </div>
      </section>
          ...
          <div className="mt-5 grid gap-5 lg:grid-cols-3">
            {activeRegion.destinations.map((destination) => (
              <article key={destination.slug} className="card-surface overflow-hidden">
                <div className="bg-navy px-6 py-5 text-white"><div className="flex items-start justify-between"><span className="text-4xl" aria-hidden>{countryFlag(destination.code)}</span><span className="rounded-full bg-white/10 px-3 py-1 text-[10px] font-bold text-gold">{destination.fitLabel}</span></div><h3 className="mt-4 text-xl font-extrabold">{destination.city}, {destination.country}</h3><p className="mt-1 text-xs text-white/60">{NEXIT_LEXICON.matchScore}: {destination.matchScore}%</p></div>
                <div className="p-6"><p className="text-sm leading-6 text-muted">{destination.summary}</p><dl className="mt-5 space-y-3 text-sm"><div><dt className="text-xs font-bold text-muted">{NEXIT_LEXICON.pathways}</dt><dd className="font-extrabold">{destination.pathways.join(' or ')}</dd></div><div><dt className="text-xs font-bold text-muted">{NEXIT_LEXICON.communityFit}</dt><dd className="font-extrabold">{destination.communityFit}</dd></div></dl><div className="mt-5 grid grid-cols-2 gap-3"><div className="rounded-xl bg-canvas p-3"><BadgeDollarSign size={17} className="text-gold-deep" /><p className="mt-2 text-[10px] text-muted">Monthly guide</p><strong className="text-sm">${destination.monthlyBudget.toLocaleString()}</strong></div><div className="rounded-xl bg-canvas p-3"><ShieldCheck size={17} className="text-gold-deep" /><p className="mt-2 text-[10px] text-muted">Research next</p><strong className="text-sm">Official rules</strong></div></div></div>
              </article>
            ))}
          </div>
        </section>
      ...
      <section className="mt-8 overflow-hidden rounded-[24px] bg-teal-deep p-6 text-white sm:p-8">
        <div className="flex flex-wrap items-end justify-between gap-5">
          <div className="max-w-2xl"><p className="flex items-center gap-2 text-xs font-extrabold uppercase tracking-[.18em] text-teal-soft"><BookOpenText size={17} />{NEXIT_LEXICON.greenbookInsights}</p><h2 className="mt-3 font-display text-3xl font-bold sm:text-4xl">Context for the life between the paperwork.</h2><p className="mt-3 text-sm leading-6 text-white/72">Editorial planning prompts about neighborhoods, workdays, arrival logistics, and questions official forms do not answer.</p></div>
          <div className="flex items-center gap-2 rounded-full bg-white/10 px-4 py-2 text-xs font-bold"><UsersRound size={16} /> Editorial preview</div>
        </div>
        ...
        <div className="mt-7 grid gap-4 lg:grid-cols-2">
          {greenbookEntries.map((entry) => (
            <article key={entry.id} className="rounded-[18px] bg-white p-5 text-navy shadow-card sm:p-6">
              ...
              <p className="mt-4 text-base font-semibold leading-relaxed">{entry.note}</p>
              <p className="mt-4 text-xs font-bold text-gold-deep">{entry.sourceLabel}</p>
