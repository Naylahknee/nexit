...
export function CountriesBrowser({ initialQuery = '' }: { initialQuery?: string }) {
  ...
  return (
    <div>
      <div><p className="text-sm font-bold text-gold-deep">Country directory</p><h1 className="mt-1 font-display text-4xl font-bold">Find a place that fits your life.</h1><p className="mt-3 max-w-2xl text-sm leading-6 text-muted">Compare each starting visa path, income threshold, estimated cost, and lifestyle fit.</p></div>
      ...
      <div className="mt-7 grid gap-5 md:grid-cols-2 xl:grid-cols-3">{filtered.map((country) => {
        ...
        return <article key={country.id} className="card-surface overflow-hidden"><div className="relative h-36 bg-gradient-to-br from-navy to-navy-deep p-5 text-white"><div className="absolute -right-5 -top-8 text-[110px] opacity-20" aria-hidden>{countryFlag(detail.code)}</div><span className="relative text-5xl" aria-hidden>{countryFlag(detail.code)}</span><p className="relative mt-3 text-xs font-bold text-gold">{detail.region}</p></div><div className="p-5"><div className="flex justify-between gap-4"><div><h2 className="text-xl font-extrabold">{country.name}</h2><p className="text-sm text-muted">{detail.city}</p></div><strong className="text-ok">{detail.match}%</strong></div><div className="mt-5 grid grid-cols-2 gap-3 rounded-xl bg-canvas p-4 text-xs"><div><span className="block text-muted">Visa</span><strong>{country.visa_type}</strong></div><div><span className="block text-muted">Income from</span><strong>${country.income_required.toLocaleString()}/mo</strong></div><div><span className="block text-muted">Cost</span><strong>{detail.cost}</strong></div><div><span className="block text-muted">Safety</span><strong>{detail.safety}</strong></div></div><Link href={`/countries/${detail.slug}`} className="gold-button mt-5 w-full">View country <ArrowRight size={16} /></Link></div></article>
      })}</div>
    </div>
  )
}
