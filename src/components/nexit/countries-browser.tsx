  ...
  return (
    <div>
      ...
      <div className="mt-7 grid gap-5 md:grid-cols-2 xl:grid-cols-3">{filtered.map((country) => {
        ...
        return <article key={country.id} className="card-surface overflow-hidden"><div className="relative h-36 bg-gradient-to-br from-navy to-navy-deep p-5 text-white"><div className="absolute -right-5 -top-8 text-[110px] opacity-20" aria-hidden>{countryFlag(detail.code)}</div><span className="relative text-5xl" aria-hidden>{countryFlag(detail.code)}</span><p className="relative mt-3 text-xs font-bold text-gold">{detail.region}</p></div><div className="p-5"><div><h2 className="text-xl font-extrabold">{country.name}</h2><p className="text-sm text-muted">{detail.city}</p></div><div className="mt-5 grid grid-cols-2 gap-3 rounded-xl bg-canvas p-4 text-xs"><div><span className="block text-muted">Pathways</span><strong>{country.visa_type}</strong></div><div><span className="block text-muted">Income guide</span><strong>${country.income_required.toLocaleString()}/mo</strong></div><div><span className="block text-muted">Cost</span><strong>{detail.cost}</strong></div><div><span className="block text-muted">Community Fit</span><strong>Research locally</strong></div></div><div className="mt-5 grid gap-2"><Link href={`/countries/${detail.slug}`} className="gold-button w-full">View Nextination <ArrowRight size={16} /></Link><SaveNextinationButton slug={detail.slug} className="w-full" /></div></div></article>
      })}</div>
    </div>
  )
}
