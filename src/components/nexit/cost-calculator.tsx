...
const BUDGET_LABELS: Record<keyof Budget, string> = { housing: 'Housing', food: 'Food', transport: 'Transport', healthcare: 'Healthcare', other: 'Other' }
...
export function CostCalculator({ income, profileComplete }: { income: number | null; profileComplete: boolean }) {
  ...
  const slices: BudgetSlice[] = (Object.keys(BUDGET_LABELS) as (keyof Budget)[])
    .map((name, index) => ({ label: BUDGET_LABELS[name], amount: budget[name] ?? 0, color: BUDGET_COLORS[index % BUDGET_COLORS.length] }))
  ...
  return <div>
    ...
    <div className="mt-7 grid gap-5 xl:grid-cols-[1fr_.75fr]"><section className="card-surface p-6"><h2 className="text-xl font-extrabold">Monthly expenses</h2><div className="mt-6 space-y-5">{(Object.keys(budget) as (keyof Budget)[]).map((name) => <label key={name} className="grid gap-2 sm:grid-cols-[1fr_180px] sm:items-center"><span className="font-bold capitalize">{name}</span><span className="relative"><span className="absolute inset-y-0 left-4 flex items-center text-muted">$</span><input aria-label={`${name} expense`} className="field pl-8" type="number" min="0" max="100000" value={budget[name] ?? ''} placeholder="Not entered" onChange={(event) => setBudget({ ...budget, [name]: event.target.value ? Math.max(0, Number(event.target.value)) : null })} /></span></label>)}</div></section>
      ...
      <section className={`rounded-card p-6 text-white ${remaining !== null && remaining < 0 ? 'bg-danger' : 'bg-navy'}`}><p className="text-sm font-bold opacity-70">After entered expenses</p><p className="mt-2 text-3xl font-extrabold">{remaining === null ? 'Not calculated' : `${remaining < 0 ? '−' : ''}$${Math.abs(remaining).toLocaleString()}`}</p></section>{slices.length ? <section className="card-surface p-6"><p className="text-xs font-bold uppercase tracking-[.16em] text-gold-deep">Nexit Cost Snapshot</p><h2 className="mt-1 text-lg font-extrabold">Monthly breakdown</h2><div className="mt-5"><BudgetDonut slices={slices} total={total} /></div></section> : null}</aside>
