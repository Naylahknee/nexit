...
export function AuthForm({ mode }: { mode: 'login' | 'signup' }) {
  ...
  return (
    <form onSubmit={submit} className="mt-8 space-y-5">
      ...
      <p className="text-center text-sm text-muted">{signup ? 'Already planning with Nexit?' : 'New to Nexit?'} <Link href={signup ? '/login' : '/signup'} className="font-extrabold text-gold-deep">{signup ? 'Sign in' : 'Create an account'}</Link></p>
    </form>
  )
}
