...
export function VisaWizard({ initial }: { initial: RelocationProfile }) {
  ...
  const panels = [
    ...
    <section key="remote"><BriefcaseBusiness className="text-gold-deep" /><p className="mt-4 text-sm font-bold uppercase tracking-[.18em] text-gold-deep">Step 2</p><h1 className="mt-2 font-display text-4xl font-bold">Can you work remotely?</h1><p className="mt-3 text-muted">This helps us identify remote-work and income-based Pathways.</p><div className="mt-8 grid grid-cols-2 gap-4">{[{ value: true, label: 'Yes, remotely' }, { value: false, label: 'No, locally' }].map((choice) => <button key={choice.label} type="button" onClick={() => setProfile({ ...profile, remote: choice.value })} className={`min-h-28 rounded-card border p-5 font-extrabold ${profile.remote === choice.value ? 'border-gold bg-gold-soft/60' : 'border-line bg-white'}`}>{choice.label}</button>)}</div></section>,
    ...
    <section key="climate"><CloudSun className="text-gold-deep" /><p className="mt-4 text-sm font-bold uppercase tracking-[.18em] text-gold-deep">Step 5</p><h1 className="mt-2 font-display text-4xl font-bold">Your Match Score is ready.</h1><p className="mt-3 text-muted">Pick a climate preference, then save your personalized Nexit Plan.</p><div className="mt-6 grid grid-cols-2 gap-2">{climates.map((climate) => <button key={climate} type="button" onClick={() => setProfile({ ...profile, climate })} className={`rounded-xl border px-3 py-3 text-sm font-bold ${profile.climate === climate ? 'border-gold bg-gold-soft/60' : 'border-line bg-white'}`}>{climate}</button>)}</div><div className="mt-6 flex items-center gap-5 rounded-card bg-navy p-5 text-white"><span className="text-4xl" aria-hidden>{countryFlag(bestMatch.code)}</span><div><p className="text-xs font-bold uppercase tracking-[.16em] text-gold">Best Match Score</p><h2 className="mt-1 text-xl font-extrabold">{bestMatch.name} · {bestMatch.visaType}</h2><p className="mt-1 text-sm text-white/65">{Math.max(65, Math.min(98, bestMatch.score))}% Match Score</p></div></div></section>,
  ]

  return (
    <div className="mx-auto max-w-3xl"><div className="mb-8 flex items-center justify-between"><div><p className="text-sm font-bold text-muted">Pathways Wizard</p><p className="text-xs text-muted">Step {step + 1} of 5</p></div><div className="flex w-40 gap-1.5">{panels.map((_, index) => <span key={index} className={`h-2 flex-1 rounded-full ${index <= step ? 'bg-gold' : 'bg-line'}`} />)}</div></div><div className="card-surface p-6 sm:p-10">{panels[step]}{error ? <p role="alert" className="mt-5 text-sm font-semibold text-danger">{error}</p> : null}<div className="mt-10 flex justify-between"><button type="button" onClick={() => setStep((value) => Math.max(0, value - 1))} disabled={step === 0} className="inline-flex min-h-12 items-center gap-2 rounded-xl px-3 font-bold text-muted disabled:opacity-0"><ArrowLeft size={17} />Back</button>{step < 4 ? <button type="button" onClick={() => setStep(step + 1)} className="gold-button">Next<ArrowRight size={17} /></button> : <button type="button" onClick={finish} disabled={saving} className="gold-button">{saving ? 'Saving…' : 'See my dashboard'}<ArrowRight size={17} /></button>}</div></div><p className="mx-auto mt-4 max-w-xl text-center text-xs leading-5 text-muted">Match Scores are for planning only. Verify current Pathways with the official immigration authority.</p></div>
  )
}
