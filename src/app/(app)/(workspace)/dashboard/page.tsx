...
const tasks = ['Passport valid', 'Research visa options', 'Open international bank account', 'Book initial accommodation']
    ...
    <div>
      ...
      <div className="mt-7 grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
        {[
          ...
          [Sparkles, 'Best visa match', best.visaType, 'View details', `/countries/${best.slug}`],
      ...
      <div className="mt-5 grid gap-5 xl:grid-cols-[1fr_1.15fr]">
        <section className="card-surface p-6"><div className="flex items-center justify-between"><div><p className="text-xs font-bold uppercase tracking-[.16em] text-gold-deep">Visa Wizard</p><h2 className="mt-1 text-xl font-extrabold">Your best starting route</h2></div><span className="text-4xl" aria-hidden>{countryFlag(best.code)}</span></div><div className="mt-6 grid gap-5 sm:grid-cols-[1fr_auto]"><div><p className="text-sm text-muted">Based on your ${profile.monthly_income.toLocaleString()} monthly income, {profile.remote ? 'remote work' : 'local-work preference'}, and interest in {profile.preferred_region.toLowerCase()}.</p><div className="mt-5 rounded-xl bg-canvas p-4"><p className="text-xs font-bold text-muted">Recommended</p><p className="mt-1 text-lg font-extrabold">{best.name} {best.visaType}</p><p className="mt-1 text-xs text-ok">Strong starting match</p></div></div><div className="grid size-24 place-items-center rounded-full" style={{ background: `conic-gradient(#f3c516 0 ${best.match}%, #e7ebf1 ${best.match}%)` }}><div className="grid size-18 place-items-center rounded-full bg-white text-xl font-extrabold">{best.match}%</div></div></div><Link href="/visa-wizard" className="gold-button mt-6 w-full">Refine my match</Link></section>
        <section className="card-surface p-6"><div className="flex items-center justify-between"><div><p className="text-xs font-bold uppercase tracking-[.16em] text-gold-deep">Top countries for you</p><h2 className="mt-1 text-xl font-extrabold">Compare your shortlist</h2></div><Link href="/countries" className="text-xs font-extrabold text-gold-deep">View all</Link></div><div className="mt-5 grid gap-3 sm:grid-cols-3">{COUNTRIES.map((country) => <Link key={country.slug} href={`/countries/${country.slug}`} className="rounded-card border border-line p-4 transition hover:-translate-y-0.5 hover:shadow-md"><span className="text-3xl" aria-hidden>{countryFlag(country.code)}</span><p className="mt-3 font-extrabold">{country.name}</p><p className="text-xs text-muted">{country.city}</p><div className="mt-4 flex justify-between text-xs"><span>{country.cost} cost</span><strong className="text-ok">{country.match}%</strong></div></Link>)}</div></section>
      </div>

      <div className="mt-5 grid gap-5 xl:grid-cols-2">
        <section className="card-surface p-6"><div className="flex items-center justify-between"><div><p className="text-xs font-bold uppercase tracking-[.16em] text-gold-deep">Move checklist</p><h2 className="mt-1 text-xl font-extrabold">Keep your plan moving</h2></div><span className="text-sm font-bold">{completed}/{tasks.length}</span></div><div className="mt-5 h-2 overflow-hidden rounded-full bg-line"><div className="h-full rounded-full bg-navy" style={{ width: `${completed / tasks.length * 100}%` }} /></div><div className="mt-5 space-y-3">{tasks.map((task) => <div key={task} className="flex items-center gap-3 text-sm"><CheckCircle2 size={18} className={profile.completed_tasks.includes(task) ? 'text-ok' : 'text-line'} fill={profile.completed_tasks.includes(task) ? 'currentColor' : 'none'} /><span>{task}</span></div>)}</div><Link href="/checklist" className="mt-6 inline-flex items-center gap-1 text-sm font-extrabold text-gold-deep">View full checklist<ArrowRight size={14} /></Link></section>
